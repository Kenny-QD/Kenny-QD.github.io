...
var palindrome = function(string) {
  // declare a new string to ignore the spaces
...
var reverseArr = function (array) {
  ...
  var reverse = [];
  // once the original array is empty
  if(array.length === 0){
    // return the reversed array
    return reverse;
  } 
  // push in the values of the values being popped out of the original array
  reverse.push(array.pop());
  // recurse once the items are put in then slice the item by 1
  reverse = reverse.concat(reverseArr(array.slice(0)));
  // return the reversed array
  return reverse;
};
...
let rMap = (array, callback) => array.length === 1 ? callback(array):
  [callback(array[0])].concat(rMap(array.slice(1), callback));

//let rMap = (callback, array=[]) => [callback(array[0])].concat(rMap(callback,array.slice(1)));
...
var capitalizeWords = function(input) {
  ...
  // capitalize the all letter in the array
  result.push(input[0].toUpperCase());
  // declare result to concat with the callback and slice by the 1 index
...
var capitalizeFirst = function(array) {
  // assign result as an empty array to be pushed
  let result = [];
  // base case
  if (array.length === 0){
    return result;
  }
  // push element in the array with the first letter capitalized, with added the rest of the word
...
// takes a str and an obj assigned to an empty object
var letterTally = function(str, obj = {}) {
  // base case
  if (str.length === 0){
    return obj;
  }
  // if the object is undefined, assign to 1
 if (obj[str[0]] === undefined) {
  ...
  // recall with the str sliced, with the obj
...
var numToText = function(str) {
    ...
    var result = "";
  //base case if the string length is zero we want to return the result
    if (str.length === 0) {
        return result;
    } else {
      // the string at the first char is assigned to currentChar
        var currentChar = str.charAt(0);
        if(nums[currentChar]) {
            result = result + nums[currentChar];
        }
        else {
            result = result + currentChar;
        }
    //return the recursive function with the result
